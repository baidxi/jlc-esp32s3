...
int sysfs_init(void)
{
    ...
    /* 初始化根节点 */
    sysfs_node_init(&sysfs_root_node.node, "sys", NULL, SYSFS_NODE_DIR);
...
int sysfs_list_dir(struct sysfs_node *node, char *buf, size_t size)
{
    ...
    list_for_each_entry(child, &node->children, sibling) {
        ...
        ret = shell_snprintf(buf + len, size - len, "%s ", child->name);
        if (ret < 0) {
            shell_mutex_give(sysfs_mutex);
            return -1;
        }
...
int sysfs_register_device(struct device *device)
{
            ...
            if (bus_node->bus == device->bus) {
                ...
                if (bus_device_dir) {
                    ...
                    if (bus_dev_node) {
                        if (device->init_name) {
                            /* 初始化链表头 */
                            INIT_LIST_HEAD(&bus_dev_node->list);
                            ...
                            list_add_tail(&bus_dev_node->list, &bus_node->devices);
                        } else {
                            shell_free(bus_dev_node);
                        }
                    }
                }
                break;
    ...
    /* 添加到设备链表 */
    list_add_tail(&dev_node->list, &sysfs_root_node.devices);
...
{
    struct sysfs_device_node *dev_node, *tmp;
    ...
    /* 查找并删除设备节点 */
    list_for_each_entry_safe(dev_node, tmp, &sysfs_root_node.devices, list) {
    ...
    if (device->bus) {
        ...
        list_for_each_entry(bus_node, &sysfs_root_node.buses, node.sibling) {
            if (bus_node->bus == device->bus) {
                /* 查找并删除总线下的设备节点 */
                struct sysfs_device_node *bus_dev_node, *bus_tmp;
                
                list_for_each_entry_safe(bus_dev_node, bus_tmp, &bus_node->devices, list) {
                    if (bus_dev_node->device == device) {
                        list_del(&bus_dev_node->list);
                        sysfs_remove_node(&bus_dev_node->node);
                        shell_free(bus_dev_node);
                        break;
                    }
                }
...
int sysfs_register_bus(struct bus_type *bus)
{
    ...
    /* 添加到总线链表 */
    list_add_tail(&bus_node->node.sibling, &sysfs_root_node.buses);
...
{
    struct sysfs_bus_node *bus_node, *tmp;
    ...
    /* 查找并删除总线节点 */
    list_for_each_entry_safe(bus_node, tmp, &sysfs_root_node.buses, node.sibling) {
        if (bus_node->bus == bus) {
            list_del(&bus_node->node.sibling);
